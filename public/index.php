...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'generate_structure') {
            ...
            if (($course['title'] ?? '') === 'Cidades Inclusivas') {
                throw new RuntimeException('Cidades Inclusivas usa estrutura oficial de 5 módulos. O gerador genérico foi bloqueado para preservar o modelo homologado.');
            }
            ...
            foreach ($defs as $moduleIndex => $def) {
                ...
                for ($lessonPosition = 1; $lessonPosition <= 2; $lessonPosition++) {
                    ...
                    $lessonStmt->execute([$moduleId, $lessonPosition, $lessonTitle, $lessonObjective, $script]);
                }
            }
            $pdo->prepare("UPDATE courses SET status='estrutura_gerada' WHERE id=?")->execute([$courseId]);
            $pdo->commit();
            flash('Estrutura gerada: 3 módulos e 6 aulas. Revise a primeira aula para liberar a Fábrica do Curso.');
        ...
        if ($action === 'review_lesson') {
            ...
            if ($status === 'aprovada') {
                $pdo->prepare("UPDATE courses SET status='primeira_aula_revisada' WHERE id=?")->execute([$courseId]);
                flash('Primeira aula aprovada. Fábrica do Curso liberada.');
                go('factory.php?course=' . $courseId);
            }
            flash('Revisão salva com status: ' . $status . '.');
...
$firstApproved = $firstLesson && $firstLesson['review_status'] === 'aprovada';
$packageDone = $course && $course['status'] === 'pacote_didatico_gerado';
$isCidadesInclusivas = $course && ($course['title'] ?? '') === 'Cidades Inclusivas';
?>
<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Cursos IA MVP — Criador de Curso</title>
<style>:root{font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#172033;background:#f4f6fa}*{box-sizing:border-box}body{margin:0}.top{background:#111a2e;color:#fff;padding:18px 28px;display:flex;justify-content:space-between;align-items:center}.badge{font-size:12px;background:#263a68;padding:7px 10px;border-radius:999px}.layout{display:grid;grid-template-columns:270px 1fr;min-height:calc(100vh - 64px)}aside{background:#fff;border-right:1px solid #e2e7f0;padding:20px}.content{padding:26px;max-width:1180px;width:100%}.card{background:#fff;border:1px solid #e2e7f0;border-radius:14px;padding:20px;margin-bottom:18px;box-shadow:0 3px 14px rgba(16,24,40,.04)}h1,h2,h3{margin-top:0}label{font-size:13px;font-weight:700;display:block;margin:12px 0 6px}input,textarea,select{width:100%;padding:11px;border:1px solid #cfd6e4;border-radius:9px;font:inherit}textarea{min-height:110px}.btn{display:inline-block;background:#182a52;color:#fff;border:0;border-radius:9px;padding:10px 14px;font-weight:700;cursor:pointer;text-decoration:none}.btn.secondary{background:#eef2f8;color:#182a52}.btn:disabled{opacity:.45;cursor:not-allowed}.muted{color:#667085;font-size:13px}.course-link{display:block;padding:10px;border-radius:8px;text-decoration:none;color:#27324a;margin-bottom:6px}.course-link.active{background:#edf2ff;font-weight:700}.steps{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0 20px}.step{font-size:12px;padding:7px 10px;border-radius:999px;background:#edf0f5}.step.done{background:#e8f7ed;color:#176b35}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.source{padding:12px;border:1px solid #e3e7ef;border-radius:10px;margin-bottom:8px}.module{border-left:4px solid #334f8f;padding-left:14px;margin:18px 0}.lesson{margin:8px 0;padding:10px;background:#f8f9fc;border-radius:8px}.flash{padding:12px 14px;border-radius:9px;margin-bottom:16px;background:#eaf7ee;color:#185f34}.flash.error{background:#fff0f0;color:#9b1c1c}.status{font-weight:700;text-transform:uppercase;font-size:11px;letter-spacing:.04em}.next{border:1px solid #cde8d5;background:#f2fbf5}.locked{padding:12px 14px;border-radius:9px;background:#eef2f8;color:#344054}@media(max-width:800px){.layout{grid-template-columns:1fr}aside{border-right:0;border-bottom:1px solid #e2e7f0}.grid{grid-template-columns:1fr}.content{padding:16px}}</style></head>
<body><div class="top"><strong>Cursos IA MVP</strong><span class="badge">HML · Criador de Curso v0.5</span></div><div class="layout"><aside><h3>Cursos</h3>
...
<hr style="border:0;border-top:1px solid #eee;margin:18px 0"><a class="btn secondary" href="./">+ Novo curso</a><p><a class="btn secondary" href="cidades_inclusivas.php">Modelo Cidades Inclusivas</a></p><?php if ($course && $firstApproved): ?><p><a class="btn secondary" href="factory.php?course=<?= $courseId ?>">Fábrica do Curso</a></p><?php endif; ?><p class="muted" style="margin-top:18px"><a href="diagnostic.php">Diagnóstico técnico</a></p></aside><main class="content">
<?php if ($flash): ?><div class="flash <?= h($flash['type']) ?>"><?= h($flash['message']) ?></div><?php endif; ?>
<?php if (!$course): ?>
<div class="card"><h1>Criar novo curso</h1><p class="muted">Cadastre o objetivo pedagógico antes das fontes.</p><p><a class="btn secondary" href="cidades_inclusivas.php">Usar modelo oficial Cidades Inclusivas</a></p><form method="post"><input type="hidden" name="action" value="create_course"><label>Título do curso</label><input name="title" required><label>Público-alvo</label><input name="audience" required><label>Objetivo principal</label><textarea name="objective" required></textarea><br><button class="btn">Criar curso e continuar</button></form></div>
<?php else: ?>
<h1><?= h($course['title']) ?></h1><p><?= h($course['objective']) ?></p>
<div class="steps"><span class="step done">1. Curso</span><span class="step <?= $sources ? 'done' : '' ?>">2. Fontes</span><span class="step <?= $modules ? 'done' : '' ?>">3. Estrutura</span><span class="step <?= $firstApproved ? 'done' : '' ?>">4. Revisão Aula 1</span><span class="step <?= $packageDone ? 'done' : '' ?>">5. Fábrica do Curso</span><span class="step">6. Vídeo/HeyGen</span><span class="step">7. Entrega</span></div>
...
<div class="card"><h2>Upload de fonte</h2><p class="muted">TXT, MD, CSV, PDF e DOCX até 10 MB. PDF digitalizado com OCR entra em etapa posterior.</p><form method="post" enctype="multipart/form-data"><input type="hidden" name="action" value="upload_source"><input type="hidden" name="course_id" value="<?= $courseId ?>"><input type="file" name="source_file" accept=".txt,.md,.csv,.pdf,.docx" required><br><br><button class="btn">Enviar e processar</button></form></div></div>
<div class="card"><h2>Fontes processadas</h2><?php if (!$sources): ?><p class="muted">Nenhuma fonte adicionada.</p><?php else: foreach ($sources as $source): ?><div class="source"><strong><?= h($source['name']) ?></strong> <span class="status">· <?= h($source['source_type']) ?> · <?= h($source['processing_status']) ?></span><p class="muted"><?= h(excerpt((string)$source['content'], 220)) ?></p></div><?php endforeach; endif; ?><?php if ($isCidadesInclusivas): ?><div class="locked"><strong>Estrutura oficial protegida:</strong> Cidades Inclusivas utiliza 5 módulos homologados. O gerador genérico está desativado para este curso.</div><?php else: ?><form method="post"><input type="hidden" name="action" value="generate_structure"><input type="hidden" name="course_id" value="<?= $courseId ?>"><button class="btn" <?= $sources ? '' : 'disabled' ?>>Gerar módulos e aulas</button></form><?php endif; ?></div>
<?php if ($modules): ?><div class="card" id="estrutura"><h2>Estrutura do curso</h2><p class="muted"><?= count($modules) ?> módulos carregados para este curso.</p><?php foreach ($modules as $module): ?><div class="module"><h3>Módulo <?= (int)$module['position'] ?> — <?= h($module['title']) ?></h3><p><?= h($module['objective']) ?></p><?php foreach ($module['lessons'] as $lesson): ?><div class="lesson"><strong>Aula <?= (int)$lesson['position'] ?> — <?= h($lesson['title']) ?></strong><br><span class="muted">Revisão: <?= h($lesson['review_status']) ?></span></div><?php endforeach; ?></div><?php endforeach; ?></div><?php endif; ?>
<?php if ($firstLesson): ?><div class="card" id="aula1"><h2>Revisão da primeira aula</h2><p class="muted">A aprovação desta aula libera a Fábrica do Curso. Vídeo continua em etapa posterior.</p><form method="post"><input type="hidden" name="action" value="review_lesson"><input type="hidden" name="course_id" value="<?= $courseId ?>"><input type="hidden" name="lesson_id" value="<?= (int)$firstLesson['id'] ?>"><label>Roteiro</label><textarea name="script" style="min-height:340px" required><?= h($firstLesson['script']) ?></textarea><label>Observações do revisor</label><textarea name="reviewer_notes"><?= h($firstLesson['reviewer_notes']) ?></textarea><label>Status</label><select name="review_status"><option value="pendente" <?= $firstLesson['review_status'] === 'pendente' ? 'selected' : '' ?>>Pendente</option><option value="ajustes" <?= $firstLesson['review_status'] === 'ajustes' ? 'selected' : '' ?>>Solicitar ajustes</option><option value="aprovada" <?= $firstLesson['review_status'] === 'aprovada' ? 'selected' : '' ?>>Aprovada — liberar Fábrica</option></select><br><br><button class="btn">Salvar revisão</button></form></div><?php endif; ?>
<?php if ($firstApproved): ?><div class="card next"><h2>Etapa 5 liberada</h2><p>Slides, apostila, exercícios, avaliação, página de venda e certificado estão disponíveis na Fábrica do Curso.</p><a class="btn" href="factory.php?course=<?= $courseId ?>">Abrir Fábrica do Curso</a></div><?php endif; ?>
