...
<hr style="border:0;border-top:1px solid #eee;margin:18px 0"><a class="btn secondary" href="./">+ Novo curso</a><?php if ($course && $firstApproved): ?><p><a class="btn secondary" href="factory.php?course=<?= $courseId ?>">Fábrica do Curso</a></p><?php endif; ?><p class="muted" style="margin-top:18px"><a href="diagnostic.php">Diagnóstico técnico</a></p></aside><main class="content">
<?php if ($flash): ?><div class="flash <?= h($flash['type']) ?>"><?= h($flash['message']) ?></div><?php endif; ?>
<?php if (!$course): ?>
<div class="card"><h1>Criar novo curso</h1><p class="muted">Cadastre o objetivo pedagógico antes das fontes.</p><form method="post"><input type="hidden" name="action" value="create_course"><label>Título do curso</label><input name="title" required><label>Público-alvo</label><input name="audience" required><label>Objetivo principal</label><textarea name="objective" required></textarea><br><button class="btn">Criar curso e continuar</button></form></div>
<?php else: ?>
...
<?php if ($modules): ?><div class="card" id="estrutura"><h2>Estrutura do curso</h2><p class="muted">3 módulos / 6 aulas gerados a partir das fontes carregadas.</p><?php foreach ($modules as $module): ?><div class="module"><h3>Módulo <?= (int)$module['position'] ?> — <?= h($module['title']) ?></h3><p><?= h($module['objective']) ?></p><?php foreach ($module['lessons'] as $lesson): ?><div class="lesson"><strong>Aula <?= (int)$lesson['position'] ?> — <?= h($lesson['title']) ?></strong><br><span class="muted">Revisão: <?= h($lesson['review_status']) ?></span></div><?php endforeach; ?></div><?php endforeach; ?></div><?php endif; ?>
