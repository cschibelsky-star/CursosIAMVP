...
function syncCreatorPedagogyStatus(PDO $pdo, int $courseId): array {
    $s=$pdo->prepare("SELECT COUNT(*) total, SUM(CASE WHEN l.review_status='aprovada' THEN 1 ELSE 0 END) approved FROM modules m INNER JOIN lessons l ON l.module_id=m.id WHERE m.course_id=?");
    $s->execute([$courseId]);
    $row=$s->fetch(PDO::FETCH_ASSOC)?:[];
    $total=(int)($row['total']??0); $approved=(int)($row['approved']??0);
    $homologated=$total>0&&$approved===$total;
    $pdo->prepare('UPDATE courses SET status=? WHERE id=?')->execute([$homologated?'homologado_pedagogico':'estrutura_gerada',$courseId]);
    return ['total'=>$total,'approved'=>$approved,'homologated'=>$homologated];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'generate_structure') {
            ...
            flash($engine==='ia'?'Estrutura gerada pela IA a partir das fontes. Revise e aprove as aulas para concluir a homologação pedagógica.':'Estrutura gerada pelo fallback. Revise e aprove as aulas antes da geração do pacote final.');
        ...
        if ($action === 'add_lesson') {
            ...
            syncCreatorPedagogyStatus($pdo,$cid);
            flash('Nova aula adicionada ao módulo. A homologação pedagógica voltou a ficar pendente.'); go('?course='.$cid.'#estrutura');
        }
        if ($action === 'delete_lesson') {
            ...
            $pdo->commit();
            $pedagogy=syncCreatorPedagogyStatus($pdo,$cid);
            flash('Aula removida e posições reorganizadas. Homologação: '.$pedagogy['approved'].'/'.$pedagogy['total'].' aulas aprovadas.'); go('?course='.$cid.'#estrutura');
        }
        if ($action === 'regenerate_module') {
            ...
            $pdo->commit();
            syncCreatorPedagogyStatus($pdo,$cid);
            flash('Módulo regenerado pela IA sem alterar os demais módulos. As aulas regeneradas precisam ser revisadas novamente.'); go('?course='.$cid.'#estrutura');
        }
        if ($action === 'review_lesson') {
            ...
            $s=$pdo->prepare('SELECT l.id FROM lessons l INNER JOIN modules m ON m.id=l.module_id WHERE l.id=? AND m.course_id=? LIMIT 1'); $s->execute([$lid,$cid]); if(!$s->fetchColumn()) throw new RuntimeException('A aula informada não pertence a este curso.');
            $s=$pdo->prepare('UPDATE lessons SET script=?,reviewer_notes=?,review_status=? WHERE id=?'); $s->execute([$script,$notes,$status,$lid]);
            $pedagogy=syncCreatorPedagogyStatus($pdo,$cid);
            flash($pedagogy['homologated']?'Revisão salva. Curso homologado pedagogicamente: todas as aulas estão aprovadas.':'Revisão salva: '.$status.'. Homologação: '.$pedagogy['approved'].'/'.$pedagogy['total'].' aulas aprovadas.'); go('?course='.$cid.'#aula1');
        }
    } catch(Throwable $e) {
...
$courseId=(int)($_GET['course']??($courses[0]['id']??0)); $course=null;$sources=[];$modules=[];$firstLesson=null;$reviewTotal=0;$reviewApproved=0;
if($courseId){$s=$pdo->prepare('SELECT * FROM courses WHERE id=?');$s->execute([$courseId]);$course=$s->fetch();$s=$pdo->prepare('SELECT * FROM sources WHERE course_id=? ORDER BY id DESC');$s->execute([$courseId]);$sources=$s->fetchAll();$s=$pdo->prepare('SELECT * FROM modules WHERE course_id=? ORDER BY position');$s->execute([$courseId]);$modules=$s->fetchAll();foreach($modules as &$m){$ls=$pdo->prepare('SELECT * FROM lessons WHERE module_id=? ORDER BY position');$ls->execute([$m['id']]);$m['lessons']=$ls->fetchAll();foreach($m['lessons'] as $lesson){$reviewTotal++;if(($lesson['review_status']??'')==='aprovada')$reviewApproved++;}if(!$firstLesson&&!empty($m['lessons']))$firstLesson=$m['lessons'][0];}unset($m);}
$pedagogyHomologated=$reviewTotal>0&&$reviewApproved===$reviewTotal;
$aiReady=aiIsReady();
...
<div class="top"><strong>Cursos IA MVP</strong><span class="badge">HML · Criador consolidado</span></div><div class="layout"><aside><h3>Cursos</h3><?php foreach($courses as $c):?><a class="course-link <?=$courseId===(int)$c['id']?'active':''?>" href="?course=<?=(int)$c['id']?>"><?=h($c['title'])?><br><span class="muted"><?=h($c['status'])?></span></a><?php endforeach;?><hr><a class="btn secondary" href="./">+ Novo curso</a><p class="muted"><a href="dashboard.php">Dashboard</a> · <a href="financial.php">Financeiro</a> · <a href="diagnostic.php">Diagnóstico</a></p></aside><main class="content">
<?php if($flash):?><div class="flash <?=h($flash['type'])?>"><?=h($flash['message'])?></div><?php endif;?>
<div class="ai <?=$aiReady?'ready':'warn'?>"><strong>Motor de IA:</strong> <?=$aiReady?'Centro IA configurado e pronto para geração pelas fontes.':'Centro IA indisponível; o fallback pedagógico mantém o fluxo operacional.'?></div>
...
<h1><?=h($course['title'])?></h1><p><?=h($course['objective'])?></p><div class="card"><div class="grid"><div><strong>Nível</strong><br><span class="muted"><?=h($course['course_level']?:'não definido')?></span></div><div><strong>Duração desejada</strong><br><span class="muted"><?= $course['desired_hours']?number_format((float)$course['desired_hours'],1,',','.').' h':'não definida' ?></span></div><div><strong>Modalidade</strong><br><span class="muted"><?=h($course['modality']?:'não definida')?></span></div><div><strong>Linguagem</strong><br><span class="muted"><?=h($course['language_style']?:'padrão didático')?></span></div></div><?php if(!empty($course['expected_outcome'])):?><p style="margin-bottom:0"><strong>Resultado esperado:</strong> <?=h($course['expected_outcome'])?></p><?php endif;?></div><div class="steps"><span class="step done">1. Briefing</span><span class="step <?=$sources?'done':''?>">2. Fontes</span><span class="step <?=$modules?'done':''?>">3. Arquitetura IA</span><span class="step <?=$pedagogyHomologated?'done':''?>">4. Homologação pedagógica <?=$reviewApproved?>/<?=$reviewTotal?></span><span class="step">5. Materiais IA</span><span class="step">6. Vídeo/HeyGen</span><span class="step">7. Entrega</span></div>
