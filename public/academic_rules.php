...
<div class="row"><div><label><strong>Frequência mínima presencial (%)</strong></label><input type="number" name="minimum_attendance_percent" min="0" max="100" step="0.01" value="<?=($rules['minimum_attendance_percent']===null||$rules['minimum_attendance_percent']==='')?'':arh((string)$rules['minimum_attendance_percent'])?>" placeholder="Sem exigência"></div><div><label><strong>Nota mínima final (0–100)</strong></label><input type="number" name="minimum_grade" min="0" max="100" step="0.01" value="<?=($rules['minimum_grade']===null||$rules['minimum_grade']==='')?'':arh((string)$rules['minimum_grade'])?>" placeholder="Sem exigência"></div></div>
<p class="muted">A frequência só é aplicada quando há carga presencial planejada. A nota mínima só é aplicada quando o curso tiver avaliações cadastradas.</p>
...
<div class="rule"><strong>Avaliações</strong><p class="muted">Cadastre avaliações, pesos e resultados no módulo próprio. A nota final é normalizada para uma escala de 0 a 100.</p><a class="btn secondary" href="assessments.php?course=<?=$courseId?>">Gerenciar avaliações</a></div>
