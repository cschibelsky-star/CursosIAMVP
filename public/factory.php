...
function buildCourseOutline(PDO $pdo, int $courseId): string
{
    ...
    $lines = []; $lastModule = null;
    foreach ($stmt->fetchAll() as $row) {
        $moduleKey = (string)$row['module_position'];
        if ($moduleKey !== $lastModule) { $lines[] = 'Módulo ' . $row['module_position'] . ' — ' . $row['module_title']; $lastModule = $moduleKey; }
        if ($row['lesson_position'] !== null) $lines[] = '  Aula ' . $row['lesson_position'] . ' — ' . $row['lesson_title'];
    }
    return implode("\n", $lines);
}
function pedagogyReviewState(PDO $pdo,int $courseId): array
{
    ...
    return ['total'=>$total,'approved'=>$approved,'adjustments'=>(int)($row['adjustments']??0),'pending'=>(int)($row['pending']??0),'homologated'=>$total>0&&$approved===$total];
}
function updatePackageStatus(PDO $pdo, int $courseId, array $types): bool
{
    $placeholders = implode(',', array_fill(0, count($types), '?'));
    $params = array_merge([$courseId], $types);
...
$pdo = factoryDb(); factoryMigrate($pdo);
...
$pedagogy = pedagogyReviewState($pdo, $courseId);
$approved = $pedagogy['homologated'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (!$approved) throw new RuntimeException('O curso precisa estar homologado pedagogicamente: todas as aulas devem estar aprovadas antes de gerar o pacote final.');
        if ($sourceContext === '') throw new RuntimeException('O curso precisa ter fontes processadas antes da geração dos materiais.');
        ...
        foreach ($selected as $type) {
            ...
            [$assetTitle, $content, $engineMode] = courseEngineGenerateMaterial($course, $outline, $sourceContext, $lessonContext, $type);
...
$stmt = $pdo->prepare('SELECT * FROM course_assets WHERE course_id=? ORDER BY id');$stmt->execute([$courseId]);$assets = $stmt->fetchAll();
$assetsByType = [];foreach ($assets as $asset) $assetsByType[$asset['asset_type']] = $asset;
$catalog = ['slides'=>'Slides','apostila'=>'Apostila','exercicios'=>'Exercícios','avaliacao'=>'Avaliação','pagina_venda'=>'Página de venda','certificado'=>'Certificado'];
$generatedCount = count(array_filter($types, fn(string $type): bool => isset($assetsByType[$type]) && ($assetsByType[$type]['status']??'') === 'gerado'));
...
<section class="card hero-status" style="margin-bottom:18px"><div><div class="info-strip"><span class="item">Homologação: <?=$pedagogy['approved']?>/<?=$pedagogy['total']?> aprovadas</span><span class="item">Ajustes: <?=$pedagogy['adjustments']?></span><span class="item">Pendentes: <?=$pedagogy['pending']?></span><span class="item">Fontes: <?=$sourceContext!==''?'OK':'pendentes'?></span><span class="item">Programa: <?=$lessonContext!==''?'OK':'pendente'?></span><span class="item">Motor: <?=h($engineStatus['mode'])?></span></div><p class="muted"><?=$approved?'Curso homologado pedagogicamente. O pacote final está liberado.':'Pacote final bloqueado até que todas as aulas estejam aprovadas.'?> <?=$engineStatus['external_ai']?'Centro IA conectado: a fábrica tenta gerar os materiais pela IA compartilhada e usa fallback editorial somente se a execução falhar.':'Centro IA indisponível: a fábrica continua operacional com o fallback editorial fundamentado nas fontes e aulas.'?></p></div><div style="min-width:210px"><span class="muted">Homologação pedagógica</span><div style="font-size:32px;font-weight:850;margin:4px 0"><?=$pedagogy['approved']?>/<?=$pedagogy['total']?></div><div class="progress"><span style="width:<?=$pedagogy['total']>0?round(($pedagogy['approved']/$pedagogy['total'])*100):0?>%"></span></div><p class="muted" style="margin-bottom:0"><?=$approved?'100% aprovado':'Revisão em andamento'?></p></div></section>
...
<?php if($packageComplete):?><section class="card" style="margin-top:18px;border-color:#cde8d5;background:#f5fbf7"><div class="section-title"><h2>Pacote didático completo</h2><span class="pill ok">6/6</span></div><p>O curso já possui todos os materiais-base. A próxima etapa é preparar a fila de vídeo, sem consumo real de créditos.</p><a class="btn" href="video_pipeline.php?course=<?=$courseId?>">Abrir pipeline de vídeo / HeyGen</a></section><?php endif;?>
